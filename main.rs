...
fn main() {
    ...
    match status {
        ...
        _ => {
            if !fs::metadata("music.zip").is_ok() {
    ...
    let date = match status {
        "LOCAL" => {
            let metadata = fs::metadata("music.zip").unwrap().modified();
    ...
    println!(
        ...
        tcb::new()
            ...
            .text_str(&*format!("DB last modified: {}", date))
            .print()
    );
